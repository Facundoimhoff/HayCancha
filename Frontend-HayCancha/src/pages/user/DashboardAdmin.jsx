...
const DashboardAdmin = () => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        setProximosTurnos(turnosFuturos.slice(0, 15));
  ...
  // --- FUNCIÓN PARA CANCELAR TURNO (ADMIN) ---
  const cancelarTurno = async (idTurno) => {
    const confirmar = window.confirm("¿Estás seguro de que querés cancelar y eliminar este turno?");
    if (!confirmar) return;
    ...
    try {
      ...
      setProximosTurnos(turnos => turnos.filter(t => t.id !== idTurno));
  ...
  const PantallaAjustes = () => (
    <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '12px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
      <Settings size={48} color="#9ca3af" style={{ margin: '0 auto 15px auto' }} />
      <h2>Configuración del Club</h2>
      <p style={{ color: '#6b7280', maxWidth: '400px', margin: '0 auto' }}>Acá construiremos el panel para cambiar precios y horarios del club.</p>
    </div>
  );
